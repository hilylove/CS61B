...
public class Room extends RectangleHelper {
    ...
    private boolean containPosition(Position p) {
        return (p.x <= position.x + width - 1 && p.x >= position.x) && (p.y <= position.y + height - 1 && p.y >= position.y);
    }
    ...
    private static void removeOverlaps(List<Room> rooms) {
        for (int i = 0; i < rooms.size(); i++) {
            for (int j = i + 1; j < rooms.size(); j++) {
                if (rooms.get(i).isOverlap(rooms.get(j)) || rooms.get(j).isOverlap(rooms.get(i))) {
                    rooms.remove(j);
                    j--;
                }
            }
    ...
    /**
     * Create four random positions on room's edge.
     */
    public Position[] randomRoomPositions(Random RANDOM) {
